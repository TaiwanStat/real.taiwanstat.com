...
def get_kind(vegetable_code:str):
    ...
    return data_dict
#--------------------------------------------------------------------------------       
def get_kind_oneday(vegetable_code:str, upload_date:datetime.date, rec_day:int = 0):
    oneday_dict = dict()
    true_day = upload_date + (oneday * rec_day)
    payload = {
        'mhidden1':'false',
    ...
    if(vg_weight_re and vg_price_re): 
        ...
        #do nat has data at this day actually.
        if(rec_day != 0):
            vg_weight = 0
        
        oneday_dict = {str(true_day) : {
                            'date':{
                                'year':true_day.year,
                                'month':true_day.month, 
                                'day':true_day.day},
                            'weight':(vg_weight),
                            'price':(vg_price)} 
                      }
    else:
        oneday_dict = get_kind_oneday(vegetable_code, upload_date - oneday, rec_day+1)
    return oneday_dict
#--------------------------------------------------------------------------------       
...
if __name__ == '__main__':
    ...
    for group in index_dict:
        ...
        #group = what we check, vegetable_dict = group's list.
